    ...
    <div class="box-free-trial">
        ...
        <div class="box-setup-profile">
            ...
            <div class="box-profile-setup">
                ...
                <div class="row mb-3">
                    <div class="col-md-12 input_box">
                        <input id="number" type="number" name="phone"
                            class="form-control  @error('phone') is-invalid @enderror" value="{{ Auth::user()->phone }}"
                ...
                <input type="file" id="card_numberInput" name="card_image" accept="image/*" style="display: none;">
                ...
                <div class="row mb-3 mt-4">
                    <div class="col-md-12 input_box">
                        <input type="text" name="line_id" class="form-control @error('line_id') is-invalid @enderror"
                            name="line_id" value="{{ Auth::user()->line_id }}" required autocomplete="Line id">
                        <label>Line ID </label>
                        @error('line_id')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                ...
                <div class="row mb-3">
                    <div class="col-md-12 input_box">
                        <input type="url" name="facebook" class="form-control @error('facebook') is-invalid @enderror"
                            name="facebook" value="{{ Auth::user()->facebook }}" required autocomplete="Facebook">
                ...
                <div class="flex-direction-row mb-3">
                    <select class="form-select" aria-label="Default select example" name="property_type">
                        <option value="บ้านเดี่ยว" @if (Auth::user()->property_type == 'บ้านเดี่ยว') selected @endif>บ้านเดี่ยว</option>
                        <option value="คอนโด" @if (Auth::user()->property_type == 'คอนโด') selected @endif>คอนโด</option>
                        <option value="ทาวน์เฮ้าส์" @if (Auth::user()->property_type == 'ทาวน์เฮ้าส์') selected @endif>ทาวน์เฮ้าส์</option>
                ...
                <div class="flex-direction-row">
                    <select class="form-select" name="characteristics" aria-label="Default select example">
                        ...
                        <option value="ทรัพย์มือหนึ่ง" @if (Auth::user()->characteristics == 'ทรัพย์มือหนึ่ง') selected @endif>ทรัพย์มือหนึ่ง
                        </option>
                        <option value="เช่าออม" @if (Auth::user()->characteristics == 'เช่าออม') selected @endif>เช่าออม</option>
                        <option value="ตกแต่งสวยเว่อร์" @if (Auth::user()->characteristics == 'ตกแต่งสวยเว่อร์') selected @endif>ตกแต่งสวยเว่อร์
                        </option>
                        <option value="เช่าระยะสั้นได้ (น้อยกว่า 6 เดือน)"
                            @if (Auth::user()->characteristics == 'เช่าระยะสั้นได้ (น้อยกว่า 6 เดือน)') selected @endif>เช่าระยะสั้นได้ (น้อยกว่า 6 เดือน)</option>
                        <option value="ใกล้มหาวิทยาลัยดัง" @if (Auth::user()->characteristics == 'ใกล้มหาวิทยาลัยดัง') selected @endif>
                            ใกล้มหาวิทยาลัยดัง</option>
                        <option value="ห้องเปล่า" @if (Auth::user()->characteristics == 'ห้องเปล่า') selected @endif>ห้องเปล่า</option>
                        <option value="ขายขาดทุน" @if (Auth::user()->characteristics == 'ขายขาดทุน') selected @endif>ขายขาดทุน</option>
